...
- The old free-form inner voice has been replaced by a structured cognitive prepass: scene appraisal, uncertain user-model hypotheses, response intentions, and observable short-horizon predictions. Deterministic compilation is the measured fast default; optional adaptive mode samples a timeout-bounded V4 Flash compiler.
- Direct human replies/mentions/DMs and reactions can resolve earlier predictions. Only non-zero explicit feedback updates memory/strategy utility; neutral continuation neither rewards nor erodes it. Bot-authored turns stay visible as context but cannot train this outcome loop.
...
- Self-model changes require distinct evidence across multiple sleep cycles. The current production policy experiment can lower per-user prediction count only after an observed counterfactual replay improves precision without measured grounding, attribution, temporal, persona-format, latency, or context regression.
- `DEVELOPMENT_SHADOW_PROVIDER=local-diversity` runs a free alternate lexical/diversity reranker over a wider pre-utility candidate pool. Structured overlap/rank metrics are logged off-path and never enter the live prompt.
...
- `/guildops list|inspect|activity|invite` is owner-only and works from a DM or any server: it inventories every guild Hikari is in, reports the bot's effective permissions, reads bounded recent activity including bot messages, and creates a one-use expiring invite only where Discord grants `CreateInstantInvite`.
- Cross-guild activity follows the configured bot-owner trust boundary: the owner can read channels the bot can read even when the owner's personal account is not yet a member of that guild.
- Owner-addressed DM turns also expose permission-checked write tools for DMs, messages/reactions, text channels, threads, roles/member assignment, timeouts/kick/ban/nicknames, and webhooks. These tools accept explicit guild/channel/member IDs and never bypass Discord permissions or role hierarchy.
- In a DM, `make me admin in guild <id>` invokes `discord_claim_administrator` for the configured owner account. The owner must have joined first; Hikari reuses a manageable Administrator role or creates `Bot Owner` when her Discord permissions and role position allow it.
