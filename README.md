...
- When Fish TTS is enabled, Hikari can generate a private Fish Speech S2.1 voice script with sparse `[bracket]` emotion tags such as `[excited]`, `[laugh]`, `[sigh]`, `[whisper]`, `[surprised]`, `[teasing]`, `[pause]`, and `[emphasis]`; Discord only sees the cleaned display text.
